...
class SpecialRequestDump extends FormSpecialPage {
  private $jobQueueGroup;  // type JobQueueGroup
  private $fileBackend;    // type FileBackend
  ...
  public function __construct (FileBackend $fileBackend, ConfigFactory $configFactory, JobQueueGroup $jobQueueGroup) {
    parent::__construct( 'RequestDump' );
    $this->fileBackend = $fileBackend;
    $this->requestLimit = $configFactory->makeConfig( 'DumpsOnDemand' )->get( 'DumpsOnDemandRequestLimit' );
  ...
  protected function alterForm( HTMLForm $form ): void {
    ...
    if ( $fileTypeMessage !== '' ) {$form->addHeaderText( $this->msg( $fileTypeMessage )->parseAsBlock() );}
  }
  ...
  protected function getFormFields(): array {
    $fields = [
      'current-revisions' => [
        ...
        'icon'          => 'download'
      ] + $this->getFormattedDate($this->fileBackend->getCurrentRevisionsFileTimestamp()),
      'all-revisions' => [
        'class' => HTMLHrefButtonField::class,
    ...
    if ( !$this->getAuthority()->isAllowed( 'dumpsondemand' ) ) {$fields['request-dump']['disabled'] = true;} 
  ...
  private function getFormattedDate( $unixTimestamp ): array {
    if ( $unixTimestamp === false ) {return ['buttonlabel-message' => 'dumpsondemand-dump-unavailable', 'disabled' => true]; }
    $this->getLanguage()->userTimeAndDate($unixTimestamp, $this->getUser());
    return ['buttonlabel' => $this->getLanguage()->userTimeAndDate($unixTimestamp, $this->getUser())];
  }
  ...
  public function onSubmit( array $data ): bool {
    ...
    $this->jobQueueGroup->push( [
      'currentRevisions' => new JobSpecification( DoDatabaseDumpJob::JOB_NAME, [ 'fullHistory' => false ], [ 'removeDuplicates' => true ] ),
      'allRevisions'     => new JobSpecification( DoDatabaseDumpJob::JOB_NAME, [ 'fullHistory' => true ], [ 'removeDuplicates' => true ] )
    ] );
  ...
  public function getGroupName():     string {return 'wiki';}
  public function getDisplayFormat(): string {return 'ooui';}
  ...
  private function userCanRequestDump(): bool {
    ...
    if ( $allRevisions < 0 || $currentRevisions < 0 ) { return true;}
