...
class DoDatabaseDumpJob extends Job implements GenericParameterJob {
  ...
  private $lbFactory;         // : Interface for generating database load balancers
  private $fileBackend;
  ...
  public function run(): bool {
    ...
    if ( $this->params['fullHistory'] ) {$file = $this->fileBackend->getAllRevisionsFilePath();} 
    else                                {$file = $this->fileBackend->getCurrentRevisionsFilePath();}

    $exporter = new WikiExporter($dbr, $this->params['fullHistory'] ? WikiExporter::FULL : WikiExporter::CURRENT);
