...
( function( mw, $ ) { $( document ).ready( function() {
  ...
  // pick up some references to the user interface
  var $pushButton      = $( '.push-button' );
  var $pushAllButton   = $( '#push-all-button' );
  var $txtTemplateList = $( '#txtTemplateList' );
  var $txtFileList     = $( '#txtFileList' );
  var $checkIncFiles   = $( '#checkIncFiles' );


  $.each( $pushButton, function( i, v ) {getRemoteArticleInfo( $( v ).attr( 'targetid' ), $( v ).attr( 'pushtarget' ) ); } );
  ...
  $pushButton.click( function() {
    ...
    if ( $('#checkIncTemplates').prop('checked') ) {pages = mw.config.get( 'wgPushTemplates' ); pages.unshift( $('#pageName').attr('value') );} 
    else {pages = [ $( '#pageName' ).attr( 'value' ) ];}

    setButtonToImgPush(this, pages, $( this ).attr( 'pushtarget' ), $( this ).attr( 'targetname' ));
  } );
  ...
  $( '#divIncTemplates' ).hover(
    function() {
      ...
      if ( isHidden ) {$txtTemplateList.css( 'display', 'inline' );}
      $txtTemplateList.fadeTo(isHidden? 'slow' : 'fast', 1);
    },
    function() {$txtTemplateList.fadeTo( 'fast', 0.5 )}
  )
  .click( function() { setIncludeFilesText(); displayTargetsConflictStatus(); });
  ...
  $( '#divIncFiles' ).click( function() {
    ...
    function() {
      ...
      if ( isHidden ) {$txtFileList.css( 'display', 'inline' ); setIncludeFilesText();}

      $txtFileList.fadeTo( isHidden ? 'slow' : 'fast', 1 );
    },
    function() {$txtFileList.fadeTo( 'fast', 0.5 )}
  );
  ...
  function setIncludeFilesText() {
    if ( $checkIncFiles.length !== 0 ) {
      ...
      if ( $('#checkIncTemplates').prop('checked') ) {files = files.concat( mw.config.get( 'wgPushTemplateFiles' ) );}
...
function getRemoteArticleInfo( targetId, targetUrl ) {
  ...
  $.getJSON(targetUrl + '/api.php?callback=?', { 'action': 'query', 'format': 'json', 'prop': 'revisions', 'rvprop': 'timestamp|user|comment',
    'titles': [pageName].concat( mw.config.get( 'wgPushTemplates' ) ).concat( mw.config.get( 'wgPushPageFiles' ) ).concat( mw.config.get( 'wgPushTemplateFiles' ) ).join( '|' ) },
      function( data ) {
        if ( data.query ) {
          ...
          for ( var remotePageId in data.query.pages ) {
            if ( data.query.pages.hasOwnProperty( remotePageId ) ) {
              if ( remotePageId > 0 ) {
                if ( data.query.pages[remotePageId].title === pageName ) {remotePage = data.query.pages[remotePageId];} else {
          ...
          if ( remotePage ) {
            ...
            message = mw.msg('push-tab-last-edit', revision.user, dateTime[0], dateTime[1].replace( 'Z', '' ) );
          } else {
  ...
  function displayTargetsConflictStatus() { $.each( $( ".push-button" ), function( i, v ) {displayTargetConflictStatus( $( v ).attr( 'targetid' ) );} ); }
...
function initiatePush ( sender, pages, targetUrl, targetName ) {
  ...
  new mw.Api().postWithEditToken( {action: 'push', page: pages, targets: targetUrl} )
    .done( function ( data ) {
      if      ( data.error )                                       {handleError( sender, data.error );     } 
      else if ( data[0] && data[0].error )                         {handleError( sender, data[0].error );  } 
      else if ( data[0] && data[0].edit && data[0].edit.captcha )  {handleError( sender, { info: mw.msg( 'push-err-captacha', targetName ) } ); } 
      else {handlePushingCompletion( sender );}  })
    .fail( function ( errorCode, data ) { handleError( sender, data.error ); });
}
...
function reEnableButton( button ) {
  ...
  if ( typeof $pushAllButton !== "undefined" ) {      // If there is a "push all" button, make sure to reset it when all other buttons have been reset.
    var hasDisabled = false;
    $.each( $pushButton, function( i, v ) { if ( v.disabled ) {hasDisabled = true;} });
    if ( !hasDisabled ) { $pushAllButton.attr( "disabled", false ); $pushAllButton.text( mw.msg( 'push-button-all' ) ); }
  }
}
...
function handleError( sender, error ) {
  ...
  if ( error.code && error.code === 'uploaddisabled' ) {error.info = mw.msg( 'push-tab-err-uploaddisabled' );}
  errorDiv.text( error.info );  errorDiv.fadeIn( );

  sender.innerHTML = mw.msg( 'push-button-failed' );
  setTimeout( function() { reEnableButton( sender ); }, 500 );
}
