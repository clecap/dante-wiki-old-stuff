...
( function( mw, $ ) { $( document ).ready( function() {
  ...
  for ( i = requestAmount; i > 0; i-- ) {initiateNextPush();}

  function initiateNextPush() {
    var page = pages.pop();
    if ( page ) {startPush( page, 0, null );} else if ( !--requestAmount ) {showCompletion();}
  }
  ...
  function appendAndScroll( item ) {
    ...
    if ( atBottom ) {box.attr( {'scrollTop': box.attr( 'scrollHeight' )} );}
  }
  ...
  function startPush( pageName, targetOffset, listItem ) {
    ...
    if ( targetOffset < targets.length ) {
      ...
      new mw.Api().postWithEditToken( { action: 'push', page: pageName, targets: targets.slice( currentBatchStart, currentBatchLimit ).join( '|' )})
      .done( function ( data ) {
        if ( data.error ) {handleError( listItem, pageName, data.error );} 
        else if ( data.length > 0 && data[0].edit && data[0].edit.captcha ) {handleError( listItem, pageName, { info: mw.msg( 'push-err-captcha-page', pageName ) } );} 
        else {startPush( pageName, targetOffset, listItem );}
      } ).fail( function ( errorCode, data ) {handleError( listItem, pageName, data.error );});
    }
    else {
      if ( mw.config.get( 'wgPushIncFiles' ) ) {getIncludedImagesAndInitPush( pageName, listItem );}
      else {completeItem( pageName, listItem );}
    }
  }
...
function getIncludedImagesAndInitPush( pageName, listItem ) {
  ...
  new mw.Api().get( { 'action': 'query', 'prop': 'images', 'titles': pageName, 'imlimit': 500})
    .done( function ( data ) {
      if ( data.query ) {
        var images = [];
      ...
      else {handleError( pageName, { info: mw.msg( 'push-special-err-imginfo-failed' ) } );}
    });
}
