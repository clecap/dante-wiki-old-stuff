...
// A special page that allows pushing one or more pages to one or more targets  Partly based on MediaWiki's Special:Export.

class SpecialPush extends SpecialPage {

  public function __construct() { parent::__construct( 'Push', 'bulkpush' ); }

  public function getDescription() {return $this->msg( 'special-' . strtolower( $this->getName() ) )->text();}
...
// main method
  public function execute( $arg ) {
    ...
    if ( !$this->userCanExecute( $this->getUser() ) ) {$this->displayRestrictionError();  return;}        // If the user is authorized, display the page, if not, show an error.
    ...
    if ( $doPush ) {$this->doPush( $pages );} else {  $this->displayPushInterface( $pages ); }
  }
  ...
  protected function doPush( $pages ) {
    ...
    } else {$targets = array_values( $egPushTargets );}
    ...
    $out->addHTML(
      ...
      Html::rawElement('div',
        ['id' => 'pushResultDiv', 'style' => 'width: 100%; height: 300px; overflow: auto' ],
        Html::rawElement( 'div',  [ 'class' => 'innerResultBox' ],   Html::element( 'ul', [ 'id' => 'pushResultList' ] )
        )
      ) . '<br />' .
  ...
  protected function displayPushInterface( $pages ) {
    ...
    $form .= Xml::inputLabel(
      $this->msg( 'export-addcattext' )->text(),'catname', 'catname', 40 ) . '&#160;';
    $form .= Xml::submitButton(
      $this->msg( 'export-addcat' )->text(),
      [ 'name' => 'addcat' ]
      ) . '<br />';

    $form .= Html::namespaceSelector( [  'selected' => $req->getText( 'nsindex', '' ),  'all' => null,  'label' => $this->msg( 'export-addnstext' )->text(),], [
      'name' => 'nsindex',
      'id' => 'namespace',
    ...
    if ( $this->getUser()->isAllowed( 'filepush' ) ) {
      $form .= Xml::checkLabel( $this->msg( 'push-special-inc-files' )->text(), 'files',  'wpPushFiles',  $req->wasPosted() ? $req->getCheck( 'files' ) : $egPushIncFiles ) . '<br />';
    }
...
protected function getPagesFromNamespace( $nsindex ) {
  $dbr = wfGetDB( DB_REPLICA );
  $res = $dbr->select('page', [ 'page_namespace', 'page_title' ], [ 'page_namespace' => $nsindex ],  __METHOD__,  [ 'LIMIT' => '5000' ] );
...
protected function getGroupName() { return 'pagetools';}
